...
## Project Overview
Loss of property has been a very serious problem that is widespread, numerous, and persistent, especially when local news reports on the recent rise in thefts and robberies over the past year or so. When people lose something to criminal activity, it is always known that there is never a guarantee that they will recover the lost object, much less recover the full value of that object. The Federal Bureau of Investigation (FBI) has a Crime Data Explorer (CDE) application that allows visitors to explore their very comprehensive database of crime statistics. One data set of interest at this time is the Cargo Theft Data. With this data, the plan is to develop a machine learning model that can predict whether a lost property is likey to be recovered depending on various factors.
